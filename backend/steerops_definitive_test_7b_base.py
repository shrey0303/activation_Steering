"""
SteerOps Definitive Pipeline Test — 7B BASE Model
=========================================================
...
async def run_definitive_test():
    ...
    # ── CONTROL TEST: Force-disable chat template ──────────
    # This simulates the ORIGINAL code path (no chat template).
    # Purpose: prove that steering works at 7B scale when the
    # model doesn't expect instruction formatting.
    if hasattr(mm.tokenizer, 'chat_template'):
        logger.info(f"Chat template FOUND on base model tokenizer — disabling for control test")
        mm.tokenizer.chat_template = None
    logger.info("Chat template status: DISABLED (control test)")
    ...
    report = {
        "title": "SteerOps Definitive Pipeline Test — 7B BASE",
    ...
    # ── Save output ─────────────────────────────────────────
    output_file = "steerops_7b_base_results.json"
