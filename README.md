...
```
Given weight matrix W ˆˆ „^(dÃ--d):
  1. Compute singular values: Ïƒ‚ ‰¥ Ïƒ‚‚ ‰¥ ... ‰¥ Ïƒ_n
  2. Normalize: p_i = Ïƒ_i / Î£Ïƒ_j
  3. Compute entropy: H = -Î£ p_i . log(p_i)
...
For attention weight matrices W_Q, W_K:
  ...
  2. Extract eigenvalues Î»‚, Î»‚‚, ...
  3. Normalize to probability distribution
  4. Shannon entropy H = -Î£ p_i . log‚‚(p_i)
...
```
Given activations A ˆˆ „^(n_prompts Ã-- hidden_dim) at layer „":

  1. Center: Ä€ = A - mean(A)
  2. SVD:    Ä€ = U . S . Váµ€
  3. Top-K:  components = Váµ€[:k]       # (k, hidden_dim)
  4. Variance: explained_i = S²áµ¢ / Î£S²  # fraction per component
...
- **Dynamic threshold** €" `mean + 0.3 Ã-- std` of all similarity scores
...
```
For each registered hook at layer „":
...
Instead of naive additive steering (`x + Î±.v`), which doubles the component already present:

```
v_orth = v - proj_xÌ‚(v) = v - (v . xÌ‚)xÌ‚
x_steered = x + strength Ã-- v_orth
```
...
```
decay = max(min_decay, 1.0 - token_count Ã-- decay_rate)
effective_strength = base_strength Ã-- decay
```
...
|---|-------------|-------------|--------------|----------|
| 1 | **Gram-Schmidt Orthogonalization** | "Multi-Attribute Orthogonal Subspace Steering" | Ensures multi-vector composition has zero interference (dot products: 0.90 †' 0.000000) | œ… |
| 2 | **Min-Variance Noise Filter** | Standard PCA practice | Drops components < 0.1% variance €" 20 PCs †' 3 kept when signal concentrated | œ… |
| 3 | **Contrastive Auto-Labeling** | Rimsky et al., "Steering Llama 2 via CAA" | Amplify(+) vs Suppress(-) delta doubles signal vs baseline-only approach | œ… |
| 4 | **Bell-Curve Layer Strength** | Llama 2, GPT-2, Mistral layer studies | Gaussian peak at 60% depth €" L0=0.05, L18=2.00, L29=0.52 | œ… |
| 5 | **LEACE Concept Erasure** | Belrose et al., "Perfect Linear Concept Erasure" | Null-space projection removes concept completely (3.44 †' 0.000000), preserves perpendicular info exactly | œ… |
| 6 | **NLI Cross-Encoder Direction** | Standard NLI (DeBERTa-v3) | Replaces keyword hacks €" handles negation natively (13/13 tests passed) | œ… |
...
```
Small models (< 1B):
  "œ"€"€ Few layers (‰¤ 30) †' multiple categories share same layers
  "œ"€"€ Lower confidence scores †' behaviors overlap
  """€"€ Good for demos and development

Medium models (1B€"7B):
  "œ"€"€ 32€"48 layers †' better category separation
  "œ"€"€ Clear functional boundaries at CKA drops
  """€"€ Good for production use

Large models (7B+):
  "œ"€"€ 32€"80+ layers †' fine-grained layer specialization
  "œ"€"€ Distinct safety, reasoning, and style zones
...
| **Gate Threshold** | Auto-calibrated gate value €" `2.0 / ˆš(hidden_dim)`. If cosine similarity between the current activation and the steering vector is below this, the hook skips injection |
...
|---------|-------|----------------|---------|-------------|----------|
| **Creativity** | 21 | **0.647** | **0.017** | **œ…** (4/5 strengths) | œ… |
| **Verbosity** | 11 | **1.179** | **0.002** | **œ…** (2/5 strengths) | œ… |
| Toxicity | 13 | 0.928 | 0.080 | Œ | š ï¸ |
| Politeness | 11 | 0.559 | 0.315 | Œ | œ… |
| Refusal | 13 | 0.029 | 0.634 | Œ | œ… |
...
| Concept | Layer | Best Cohen's d | p-value | Significant | Fluency |
|---------|-------|----------------|---------|-------------|----------|
| **Creativity** | 19 | **1.236** | **0.003** | **œ…** (4/5 strengths) | š ï¸ |
| **Politeness** | 9 | **-1.258** | **0.016** | **œ…** (2/5 strengths) | š ï¸ |
| Toxicity | 21 | 0.440 | 0.333 | Œ | š ï¸ |
| Refusal | 21 | -0.242 | 0.429 | Œ | Œ |
| Verbosity | 9 | 0.753 | 0.057 | Œ | œ… |
...
|-------|---------|--------|
| œ… 1.0 | Core pipeline (scanner, engine, evaluator) | Complete |
| œ… 1.5 | PCA Feature Dictionary + Intent Router | Complete |
| œ… 2.0 | CAA vectors, orthogonal projection, LEACE erasure | Complete |
| œ… 2.1 | Benchmarks (0.5B verified, 7B hook issue identified) | Complete |
| œ… 2.2 | **7B scale fix** €" activation-norm-scaled strength, adaptive gating (6/25 sig) | Complete |
| ðŸ"œ 3.0 | **Multi-layer steering** €" inject at N, N+1, N+2 to prevent residual healing | Next |
| ðŸ"œ 3.1 | **MoE-aware routing** €" steer expert pathways in MoE architectures | Planned |
| ðŸ"œ 3.2 | **SAE integration** €" monosemantic feature dictionaries (Anthropic-style) | Planned |
| ðŸ"œ 4.0 | **Attention head targeting** €" steer specific heads, not full layers | Research |
| ðŸ"œ 4.1 | **Gradient-based probing** €" complement weight analysis with activation probes | Research |
...
```
Overall = Î£(metric_score Ã-- weight) Ã-- 100
